        ...
        try {
          let resUser = await orbis.connect_v2({
           ...
           if(resUser.status == 200) {
             ...
             //callbackConnect(resUser.details.did);
           } else {
             setLoading(false);
           }
        } catch(e) {
  ...
  return(
    <View style={tailwind('w-full h-full')}>
      ...
      <View style={[tailwind('w-full h-full absolute px-8 items-center'), {paddingTop: 350}]}>
        ...
        <View style={[tailwind('absolute w-full'), {bottom: 150}]}>
      ...
      {connectModalVis &&
        <Modal hide={() => setConnectModalVis(false)}>
          <View style={[tailwind('flex flex-col w-full p-4 pb-12')]}>
            <LoginStep step={step} setStep={setStep} loading={loading} setLoading={setLoading} />
          </View>
        </Modal>
      }
...
const LoginStep = ({step, setStep, loading, setLoading}) => {
  const { user, setUser, orbis } = useContext(GlobalContext);
  const tailwind = useTailwind();
  ...
  async function connectWithGoogle() {
    Haptics.selectionAsync();
    let result = await WebBrowser.openBrowserAsync('https://lit.orbis.club/oauth-google/' + encodeURIComponent("exp://192.168.1.123:19000/"));
  }
