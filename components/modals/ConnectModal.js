...
import { Platform, ActivityIndicator, StyleSheet, Text, View, ScrollView, Image, SafeAreaView, TouchableOpacity, Dimensions, TextInput } from 'react-native';
...
import { AppleIcon, GoogleIcon, WalletConnectIcon } from "../Icons";
...
export default function ConnectModal({hide}) {
  ...
  useEffect(() => {
    ...
    //console.log("with provider:", provider)
      ...
      let resUser = await orbis.connect_v2({
       ...
       if(resUser.status == 200) {
         ...
         if(retryCount < 3) {
           ...
           console.log("_retryCount:", _retryCount);
